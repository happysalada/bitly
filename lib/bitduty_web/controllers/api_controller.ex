defmodule BitdutyWeb.ApiController do
  ...
  def transactions(conn, %{"account_id" => account_id}) do
    with token when not is_nil(token) <- get_session(conn, :access_token),
      {:ok, %HTTPoison.Response{status_code: 200, body: body}} <-
           HTTPoison.get("https://api.coinbase.com/v2/accounts/#{account_id}/transactions", ["Authorization": "Bearer #{token}", "Accept": "Application/json; Charset=utf-8"], [ssl: [{:versions, [:'tlsv1.2']}], recv_timeout: 500]
           ) do
      conn
      |> put_status(200)
      |> json(%{
